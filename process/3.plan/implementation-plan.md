...
| TASK-JUC-106 | **Active FM signal-path overlay.** New lightweight, **mouse-transparent** overlay `Component` painted above the static vector background (BackgroundRenderer untouched — no mockup-pipeline change), drawing **only** the branch that routes to the selected `FM_DESTINATION` (VCO1 FREQUENCY branch or VCF FREQ branch) in the knob-LED accent colour (ADR-JUC-011 SoT, same accent as the matrix highlight RQ-GUI-018); inactive branch left to the static `diagramFrame`. Branch coordinates **shared** with `BackgroundRenderer` (single-sourced, not re-measured); stroke width/rounding from diagram tokens. Repaints live on `FM_DESTINATION` change (panel + automation/synth, RQ-GUI-006) and follows the LED-colour setting live (no cached copy). **Status: Not Started.** *Acceptance (Gherkin):* Given destination `VCO1`, When painted, Then the VCO1-FREQUENCY branch is in the accent colour and the VCF branch stays neutral; Given switched to `VCF`, Then emphasis moves to the VCF-FREQ branch; Given the LED colour changes in settings, Then the active branch uses the new colour immediately. Dependencies: none hard (works against the parameter regardless of selector type); recommended sequence after TASK-JUC-105. | M | RQ-GUI-039; ADR-JUC-016 (DEC-JUC-017/018/019), ADR-JUC-011, ADR-JUC-014 |
