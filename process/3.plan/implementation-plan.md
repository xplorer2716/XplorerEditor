...
| TASK-JUC-080 | Vector background (added 2026-07, session goal): replace `main-background.jpg` by a JUCE-drawn vector background per ADR-JUC-013 — declarative `BackgroundSpec` in `xpl_app_core` (boxes/lines/stubs/captions/sections, stubs anchored to control-table knob centres), direct `Graphics` paint (no image buffering under the canvas transform), procedural brushed-metal/wood textures with fixed seed; drop the JPEG from BinaryData on cut-over. **Status: Not Started — mockup (ADR-JUC-013-mockup.svg) awaiting owner validation.** | L | RQ-GUI-037, RQ-GUI-005 |
