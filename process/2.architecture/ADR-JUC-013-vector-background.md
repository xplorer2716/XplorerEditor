...
A full-fidelity SVG mockup (`juce/tools/background-mockup.svg`, generated by
`juce/tools/generate_background_mockup.py` from the measured geometry)
demonstrates the target rendering; every SVG primitive used maps 1:1 to a
`juce::Graphics` call. *(Both moved out of `process/` to `juce/tools/` and the
generator now reads its palette/fonts from `design-tokens.yaml` — ADR-JUC-015;
the geometry is unchanged and the regenerated SVG is byte-identical.)*
...
1. **Geometry lives in the app painter, not a `BackgroundSpec` in `xpl_app_core`.**
   The owner-validated mockup (`juce/tools/background-mockup.svg` /
   `juce/tools/generate_background_mockup.py`) is the single validated source of truth
