...
namespace xplorer::app::tokens
{
    ...
    namespace global
    {
        ...
        inline constexpr int sectionGapAbove = 16;  // vertical rhythm of the canvas sections: canvas px between the BOTTOM of a section's lowest visible element and the label baseline of that section's separator bar. The bar is a TERMINATOR — its label names the section that just ended (ADR-JUC-034 DEC-JUC-107) — so it must sit closer to its own section than to the next one; this token is the "closer" half of that relation, sectionGapBelowMin the other. RQ-CLR-001, ADR-CLR-001 (DEC-CLR-001-B). Value 16, on the 4px base unit of RQ-DSN-020. DEVIATION from the owner mockup, which measured 20 (median of seven hand-placed bars): 20 does not fit the centre column's budget — between the VCF group's bottom and the RAMP X baseline, 633 px hold 508 px of incompressible group height, leaving (125 - 4*gap)/3 per below-gap, so gap must stay under 17.9 for sectionGapBelowMin > sectionGapAbove to hold. At 20 the centre column keeps the very inversion this rhythm exists to remove. Owner reviewed both values rendered from the real screenshot and chose 16. Raising this above 17 will fail SectionRhythmTests
        inline constexpr int sectionGapBelowMin = 20;  // floor for the gap between a separator's label baseline and the TOP of the next section's first visible element. A floor, not a fixed value: each column's leftover space is divided evenly among its below-gaps (RQ-CLR-004), and the two columns have very different leftovers (~45 px per gap on the left, ~20 in the centre) because they hold different amounts of content between the same two anchors. What must hold everywhere is the RELATION sectionGapBelowMin > sectionGapAbove — that is what makes the bar read as belonging to the section above it. RQ-CLR-002, ADR-CLR-001 (DEC-CLR-001-B)
    ...
    namespace component
    {
        ...
        inline constexpr int sectionGapAbove = global::sectionGapAbove;  // canvas px from a section's lowest visible element down to its separator's label baseline, RQ-CLR-001
        inline constexpr int sectionGapBelowMin = global::sectionGapBelowMin;  // floor for the gap from a separator's label baseline to the next section's first element, RQ-CLR-002
