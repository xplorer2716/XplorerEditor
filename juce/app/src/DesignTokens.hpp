...
namespace xplorer::app::tokens
{
    ...
    namespace global
    {
        ...
        inline constexpr int shortcutButtonSize = 29;  // side of a shortcut key, canvas px. NOT a free choice: RQ-GUI-065 requires the row of eight to start on the VFD left edge and finish on its right, and the VFD is 267 px wide - 8*29 + 7*5 = 267 is the ONLY integer solution for eight squares with equal gaps. Changing either number breaks that alignment; changing the VFD width forces both to be re-derived. ADR-GUI-001 (DEC-GUI-001-D)
        inline constexpr int shortcutButtonGap = 5;  // gap between two shortcut keys - the other half of the 8*29 + 7*5 = 267 solution above. RQ-GUI-065
