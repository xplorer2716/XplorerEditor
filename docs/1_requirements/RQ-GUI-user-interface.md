...
- **RQ-GUI-031** — Skin phase (after functional parity): a custom `LookAndFeel` shall reproduce the reference control appearance — knobs with configurable LED ring/border color, linear/rotary mouse modes and standard/vintage styles (UiConfiguration, RQ-SET-003), image shortcut buttons (existing GIF assets), custom checkboxes/radio panels, VFD-style display.
