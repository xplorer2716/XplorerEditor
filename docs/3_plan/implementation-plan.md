...
| TASK-JUCE-075 | Rotary knob cosmetics + page-selector captions (added 2026-07 after M2/M3 review): selector buttons show `ENV 1`…/`TRACK 3` (extracted `.label`, instance tracked separately from the caption); rotary rendered as LED-ring-only (no centre pointer) with a hover-brightened ring; knob text box removed → value as a drag popup bubble + double-click inline numeric entry, rotary fills its full bounds (larger). Per ADR-009. *Awaiting owner go-ahead.* | M | RQ-GUI-011, RQ-GUI-031, RQ-GUI-034 |
| TASK-JUCE-076 | Modulation-matrix hover highlight + VFD active-destination marker (added 2026-07 after M2/M3 review): headless `ModulationDestinationResolver` + page-selector→source map (`xpl_app_core`, unit-tested); `ModMatrixPanel` highlight API; mouse enter/exit wiring from selectors and knobs; and the deferred VFD `.` marker resolved with the same resolver. Per ADR-010 (closes analysis gap §10-12). | L | RQ-GUI-018, RQ-GUI-020 |
| TASK-JUCE-077 | LED-colour single source of truth (added 2026-07 after M3 review): the LookAndFeel becomes the sole runtime owner of the knob LED colour (`ledColour()` accessor); `ModMatrixPanel` derives the highlight colour from it instead of caching a copy, so changing the colour in the settings updates the matrix highlight live. Removes the duplicated colour state. Per ADR-011. *Awaiting owner go-ahead.* | M | RQ-GUI-018, RQ-GUI-031 |
| TASK-JUCE-078 | MIDI CC automation table — load path + editor (added 2026-07 after M3 review): (1) generate `GeneratedControlChangeNames.inc` (129 CC names) from the reference; (2) load the persisted `"NAME;CC"` table into the controller dictionary on startup/settings-apply (headless-tested parser) — currently missing, so CC automation and the VFD CC line do not work; (3) add the parameter↔CC mapping editor (list + per-row CC combo + reset) to the MIDI settings page. Per ADR-012. *Awaiting owner go-ahead.* | L | RQ-GUI-036, RQ-SET-002 |
